...
// Simple Success Modal Component specific for Dashboard
    ...
    <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
        <div className="bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-2xl flex flex-col items-center animate-fade-in max-w-sm text-center">
            ...
            <p className="text-gray-500 dark:text-gray-300 mb-6">
                Sua assinatura foi renovada com sucesso. Obrigado por confiar na FluxoClean.
            </p>
            <button 
                onClick={onClose}
...
const Dashboard: React.FC<DashboardProps> = ({ transactions, ticketSales, products, goals, onSaveGoals }) => {
    ...
    useEffect(() => {
        const checkPaymentReturn = async () => {
            ...
            // O MP retorna 'collection_status' ou 'status' (dependendo do fluxo)
            // Também retorna 'external_reference' que é o nosso ID de transação
            ...
            if (status === 'approved' && ref) {
                ...
                try {
                    // Chama endpoint que valida no MP e atualiza o banco
                    const response = await fetch(`${SAAS_API_URL}/subscription/check-payment/${ref}`, {
                        method: 'POST',
                    ...
                    if (data.status === 'approved') {
                        ...
                        // Limpa a URL para remover os parâmetros
                        window.history.replaceState({}, document.title, window.location.pathname);
                    } else {
                        console.warn("Pagamento ainda não confirmado na API.");
                    }
                } catch (error) {
    ...
    const { kpis, top10SoldProducts, top10LowestTurnoverProducts, top5SalesPeaks } = useMemo(() => {
        ...
        // Create date range covering the entire month for FINANCIAL data
        ...
        // Inventory KPI Calculations (Based on current month sales data in dashboard context)
        ...
        // --- STRATEGIC MARGIN METRICS (Real-Time / Configured Period) ---
        // Logic reused from Products.tsx to maintain consistency with Global Rules
        let periodDays = 30;
        ...
        // Calculation for "Margem Estoque/Produto" Card
        let sumActiveMargins = 0; // For Ruptura, Risco, Seguranca
        let countActiveProducts = 0;
        let sumTotalMargins = 0;
        let countTotalProducts = products.length;

        // Stock Level Summary (For the Dashboard Card based on current monthly view)
        // NOTE: Using the same logic as Products table to calculate status for this map
        const stockLevelSummary: StockLevelSummary = { ruptura: 0, risco: 0, seguranca: 0, excesso: 0 };

        products.forEach(p => {
            // 1. Calculate Real Margin (Per Product)
            ...
            // 2. Determine Status (Based on global turnover rules)
            ...
            // 3. Filter for "Active" products (Ruptura, Risco, Segurança) for the card
        ...
        // Top & Low performing products (Based on monthly sales for visual chart)
        // Reuse map for THIS MONTH ONLY calculation for other cards
    ...
    return (
        <div className="space-y-6">
            ...
            {showPaymentSuccess && <PaymentSuccessModal onClose={handleSuccessClose} />}
                        ...
                        {top10SoldProducts.length === 0 ? <p className="text-gray-500">Nenhuma venda de produto no período.</p> : top10SoldProducts.map((p, i) => (
                            <div key={p.id} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                <div className="flex items-center overflow-hidden">
                                    <span className="text-sm font-bold w-8 flex-shrink-0">{i + 1}.</span>
                                    <div className="truncate">
                                        <p className="font-medium text-sm truncate">{p.name}</p>
                                        <p className="text-xs text-gray-400">Estoque: {p.currentStock}</p>
                                    </div>
                                </div>
                                <p className="font-bold text-indigo-500 ml-2 flex-shrink-0">{p.quantitySold} <span className="font-normal text-xs">unid.</span></p>
                            </div>
                        ))}
                    </div>
                </div>
                     ...
                     <div className="space-y-3 max-h-96 overflow-y-auto">
                        {top10LowestTurnoverProducts.length === 0 ? <p className="text-gray-500">Nenhum produto com estoque.</p> : top10LowestTurnoverProducts.map((p, i) => (
                             <div key={p.id} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                ...
                                <p className={`font-bold text-xs flex-shrink-0 ml-2 ${p.turnoverRatio! === 0 ? 'text-red-500' : 'text-yellow-500'}`}>Giro: {p.turnoverRatio!.toFixed(2)}</p>
                            </div>
                        ))}
                    </div>
                </div>
