...
const ProductModal: React.FC<ProductModalProps> = ({ productToEdit, existingProducts, onClose, onSave }) => {
    ...
    useEffect(() => {
        // Only validate if we have a barcode and it's NOT the product currently being edited
        // (Typically ID is disabled on edit, but this is safe practice)
        if (barcode) {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input 
                            ...
                            className={`mt-1 block w-full rounded-md bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-3 py-2 disabled:opacity-50 ${errors.barcode ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}`}
                        />
