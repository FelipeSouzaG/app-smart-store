...
const CostPaymentModal: React.FC<{ order: ServiceOrder, accounts: FinancialAccount[], onClose: () => void, onConfirm: (costDetails: any) => void }> = ({ order, accounts, onClose, onConfirm }) => {
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md animate-fade-in">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <select value={status} onChange={(e) => setStatus(e.target.value as TransactionStatus)} className="w-full rounded p-2 bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600">
                    ...
                    {status === TransactionStatus.PENDING && (
                        <div><label className="block text-sm font-medium mb-1">Vencimento</label><input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full rounded p-2 bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600" /></div>
                    )}
                    ...
                    {status === TransactionStatus.PAID && (
                        <>
                            <div>
                                ...
                                <select value={selectedAccountId} onChange={(e) => { setSelectedAccountId(e.target.value); setSelectedMethodId(''); }} className="w-full rounded p-2 bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600">
                            ...
                            {!isCashBox && (
                                <div>
                                    ...
                                    <select value={selectedMethodId} onChange={(e) => setSelectedMethodId(e.target.value)} className="w-full rounded p-2 bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600">
                            ...
                            {isCreditCard && (
                                <div className="bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded border border-yellow-200 dark:border-yellow-700">
                                    ...
                                    <select value={installments} onChange={(e) => setInstallments(Number(e.target.value))} className="w-full rounded p-2 bg-white dark:bg-gray-800 border-yellow-300 dark:border-yellow-600">
...
const OSModal: React.FC<OSModalProps> = ({ services, orderToEdit, onClose, onSave }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setError('');
        if (!customerName || !customerWhatsapp || !selectedServiceId) { setError('Preencha os campos obrigatórios.'); return; }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <select value={selectedServiceId} onChange={handleServiceChange} className="w-full p-2 rounded border"><option value="">Selecione Serviço...</option>{services.map(s => <option key={s.id} value={s.id}>{s.name} - {s.brand} {s.model}</option>)}</select>
                    ...
                    <div className="grid grid-cols-2 gap-4"><input type="text" placeholder="Preço (R$)" value={totalPrice} onChange={e => setTotalPrice(e.target.value)} className="w-full p-2 rounded border" /><input type="text" placeholder="Outros Custos (R$)" value={otherCosts} onChange={e => setOtherCosts(e.target.value)} className="w-full p-2 rounded border" /></div>
                    {error && <p className="text-red-500 text-sm">{error}</p>}
                    <div className="flex justify-end gap-3 mt-4"><button type="button" onClick={onClose} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded">Cancelar</button><button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded font-bold hover:bg-blue-700">Salvar OS</button></div>
                </form>
            </div>
        </div>
    );
...
interface ServiceOrdersProps { services: Service[]; serviceOrders: ServiceOrder[]; onAddServiceOrder: (data: any) => Promise<void>; onUpdateServiceOrder: (data: any) => Promise<void>; onDeleteServiceOrder: (id: string) => Promise<void>; onToggleStatus: (id: string, paymentData?: any) => Promise<void>; setActivePage: (page: string) => void; goals: KpiGoals; }
const ServiceOrders: React.FC<ServiceOrdersProps> = ({ services, serviceOrders, onAddServiceOrder, onUpdateServiceOrder, onDeleteServiceOrder, onToggleStatus }) => {
    ...
    useEffect(() => { const fetchAccounts = async () => { const data = await apiCall('financial', 'GET'); if (data) setAccounts(data); }; fetchAccounts(); }, []);
    const handleEdit = (order: ServiceOrder) => { setEditingOrder(order); setIsModalOpen(true); };
    const handleDelete = async (id: string) => { if(window.confirm('Excluir OS?')) await onDeleteServiceOrder(id); };
    const handleToggle = async (order: ServiceOrder) => { if (order.status === ServiceOrderStatus.PENDING && order.totalCost > 0) { setCompletingOrder(order); } else { await onToggleStatus(order.id); } };
    const confirmCostPayment = async (costDetails: any) => { if (completingOrder) { await onToggleStatus(completingOrder.id, { costPaymentDetails: costDetails }); setCompletingOrder(null); } };

    return (
        <div className="container mx-auto">
            {isModalOpen && <OSModal services={services} orderToEdit={editingOrder} onClose={() => setIsModalOpen(false)} onSave={async (data) => { if (data.id) await onUpdateServiceOrder(data); else await onAddServiceOrder(data); }} />}
            ...
            <div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold dark:text-white">Ordens de Serviço</h1><button onClick={() => { setEditingOrder(null); setIsModalOpen(true); }} className="px-4 py-2 bg-indigo-600 text-white rounded-lg shadow-md">+ Nova OS</button></div>
            <div className="grid gap-4">{serviceOrders.map(order => (<div key={order.id} className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow border-l-4 border-indigo-500 flex justify-between items-center"><div><h3 className="font-bold text-lg">{order.serviceDescription}</h3><p className="text-sm text-gray-500">{order.customerName} - {order.customerWhatsapp}</p><p className="text-xs text-gray-400 mt-1">ID: {order.id}</p></div><div className="text-right"><p className="font-bold text-lg text-indigo-600">R$ {formatCurrencyNumber(order.totalPrice)}</p><div className="flex gap-2 mt-2"><button onClick={() => handleToggle(order)} className={`px-3 py-1 text-xs font-bold rounded ${order.status === 'Concluído' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{order.status}</button><button onClick={() => handleEdit(order)} className="text-blue-500 text-sm hover:underline">Editar</button><button onClick={() => handleDelete(order.id)} className="text-red-500 text-sm hover:underline">Excluir</button></div></div></div>))}</div>
        </div>
    );
};
